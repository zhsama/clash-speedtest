...
export default function TestHistoryModal({ onClose }: TestHistoryModalProps) {
  ...
  const handleSelectAll = () => {
    setSelectedSessions(
      selectedSessions.length === sessions.length ? [] : sessions.map((s) => s.id)
    )
  }
      ...
      <DialogContent className="max-w-6xl max-h-[90vh] flex flex-col">
        <DialogHeader className="flex-shrink-0">
          <DialogTitle className="flex items-center gap-2">
            ...
            测试历史记录
            {sessions.length > 0 && (
              <Badge className="badge-filled ml-2">{sessions.length} 条记录</Badge>
            )}
          </DialogTitle>
        </DialogHeader>
            ...
            <div className="flex justify-between items-center p-4 bg-card rounded-md">
              <div className="flex items-center gap-6 text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium">{storageStats?.sizeMB || 0} MB</span>
                </div>

                {selectedSessions.length > 0 && (
                  <div className="flex items-center gap-2">
                    <Badge className="badge-outlined">已选择 {selectedSessions.length} 项</Badge>
                  </div>
                )}
              </div>
          ...
          {sessions.length === 0 ? (
            ...
            <>
              ...
              <div className="flex items-center gap-2 px-2">
                ...
                />
                <span className="text-sm text-muted-foreground">全选 ({sessions.length} 项)</span>
              </div>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 space-y-3">
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex items-center gap-2 flex-1">
                              ...
                              <span
                                ...
                                title={session.configPaths}
                              >
                                {session.configPaths.split("/").pop() || "配置文件"}
                              </span>
                              <Badge className="badge-filled">{session.totalProxies} 节点</Badge>
                              <Badge className="badge-outlined">
                                {session.testType === "speed"
                                  ? "测速"
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            {(session.testType === "speed" || session.testType === "both") && (
                              <>
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <span
                                    className="font-medium text-warning truncate max-w-24"
                                    title={session.bestProxy}
                                  >
                            ...
                            {(session.testType === "unlock" || session.testType === "both") &&
                              session.unlockStats && (
                                <>
                                  {/* 解锁平台数 */}
                                  <div className="flex items-center gap-2">
                          ...
                          <div className="flex items-center justify-between text-xs text-muted-foreground">
                            <div className="flex items-center gap-4">
                              ...
                              {/* 不同测试类型显示不同的时长标签 */}
                              {session.testType !== "unlock" && <span>⏱️ {session.duration}</span>}
                            </div>
