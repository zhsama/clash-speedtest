...
func TestParamount(client *http.Client) *unlock.StreamResult {
	...
	case strings.Contains(htmlContent, "redirect"):
		// 检查是否重定向到特定地区
		if strings.Contains(htmlContent, "paramount") {
...
func init() {
	// 注册 Paramount+ 测试
	unlock.StreamTests = append(unlock.StreamTests, TestParamount)
}