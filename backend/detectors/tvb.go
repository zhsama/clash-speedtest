...
// TestTVB 测试 TVB 解锁情况
func TestTVB(client *http.Client) *unlock.StreamResult {
	result := &unlock.StreamResult{
		Platform: "TVB",
	}

	req, err := http.NewRequest("GET", "https://www.tvb.com/", nil)
	if err != nil {
...
func init() {
	// 注册 TVB 测试
	unlock.StreamTests = append(unlock.StreamTests, TestTVB)
}