...
func TestTVer(client *http.Client) *unlock.StreamResult {
	result := &unlock.StreamResult{
		Platform: "TVer",
	}

	req, err := http.NewRequest("GET", "https://tver.jp/", nil)
	if err != nil {
	...
	// 检查是否显示正常内容
	if strings.Contains(htmlContent, "tver") ||
		strings.Contains(htmlContent, "番組") ||
		strings.Contains(htmlContent, "ドラマ") ||
		strings.Contains(htmlContent, "バラエティ") ||
		strings.Contains(htmlContent, "テレビ") {
		result.Status = "Success"
		result.Region = "JP"
		return result
	}
...
func init() {
	// 注册 TVer 测试
	unlock.StreamTests = append(unlock.StreamTests, TestTVer)
}