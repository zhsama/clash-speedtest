...
export function useTestResultSaver(options: UseTestResultSaverOptions = {}) {
  ...
  const performCleanup = useCallback(
    (sessions: SavedTestSession[]): SavedTestSession[] => {
      const now = Date.now()
  ...
  const getSessionById = useCallback(
    (id: string): SavedTestSession | null => {
      const data = getStoredSessions()
  ...
  const getStorageStats = useCallback(() => {
    ...
    return {
      ...
      oldestSession:
        data.sessions.length > 0 ? Math.min(...data.sessions.map((s) => s.savedAt)) : null,
      newestSession:
        data.sessions.length > 0 ? Math.max(...data.sessions.map((s) => s.savedAt)) : null,
    }
  }, [getStoredSessions])

  // 导出数据
  const exportSessions = useCallback(
    (ids?: string[], format: "json" | "csv" = "json") => {
      const data = getStoredSessions()
      const sessionsToExport = ids ? data.sessions.filter((s) => ids.includes(s.id)) : data.sessions
